...
#define READ_SIZE 4096
  ...
  static void try_lock_image(ImageCtx *ictx)
  {
    // need to upgrade to a write lock
    assert(ictx->leader_lock.is_locked() && !ictx->leader_lock.is_wlocked());
    ictx->leader_lock.put_read();
    {
      RWLock::WLocker l(ictx->leader_lock);
      if (!ictx->image_watcher->is_leader()) {
	ictx->image_watcher->try_lock();
      }
    }
...
reprotect_and_return_err:
  ...
  int is_exclusive_leader(ImageCtx *ictx, bool *is_leader)
  {
    *is_leader = (ictx->image_watcher != NULL &&
		  ictx->image_watcher->is_leader());
    return 0;
  }
  ...
  public:
	...
	return;
      ...
      AsyncCompletion *completion = new AsyncCompletion(m_ctx);
      for (vector<ObjectExtent>::iterator p = extents.begin();
	   p != extents.end(); ++p) {
	ldout(m_ictx->cct, 20) << " ex " << *p << dendl;
	Context *req_comp = new C_AsyncCompletion(*completion);
	librados::AioCompletion *rados_completion =
	  librados::Rados::aio_create_completion(req_comp, NULL, rados_ctx_cb);
	if (p->offset == 0) {
		       ...
		       uint64_t new_size, ProgressContext& prog_ctx)
  {
    ...
    uint64_t num_periods = (original_size + period - 1) / period;
    uint64_t num_objects = num_periods * ictx->get_stripe_count();
  ...
  int _snap_set(ImageCtx *ictx, const char *snap_name)
  {
	...
	if (r < 0) {
      ...
      } else if (ictx->image_watcher->lock_supported() &&
	         !ictx->image_watcher->is_leader()) {
	r = ictx->image_watcher->try_lock();
	if (r < 0) {
	  lderr(ictx->cct) << "error locking image: "
			   << cpp_strerror(r) << dendl;
	}
      }
    }
