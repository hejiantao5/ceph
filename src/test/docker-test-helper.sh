...
function setup_container() {
    ...
    if ! docker images $image | grep --quiet "^$image " ; then
        # 
        # In the dockerfile,
        # replace environment variables $FOO with their content except \$FOO
        # leave non environment variables substitution untouched
        #
        cat test/$os_type.dockerfile | \
            os_version=$os_version user_id=$(id -u) \
            perl -p -e 'foreach $var (keys %ENV) { s/(?<!\\)\$($var)/$ENV{$1}/g }; s/\\(\$\w+)/$1/g;' | \
...
function run_in_docker() {
    ...
    local cmd="docker run $opts --rm --name $image --privileged $ccache --volume $downstream:$downstream --env PATH=:$PATH"
    local status=0
